...
function loadDwrIfNeeded(configuration: BrokerConfiguration): Promise<void> {
    return new Promise<void>(resolve => {
        if ((window as any).dwr) {
            shimDwrThenResolve(configuration, resolve);
        } else {
            const path = `${configuration.path}/engine.js`;
            const tag: HTMLScriptElement = document.createElement('script');
            tag.src = path;
            tag.type = 'text/javascript';
            tag.onload = () => shimDwrThenResolve(configuration, resolve);
            tag.onerror = () => shimDwrThenResolve(configuration, resolve);
            document.body.appendChild(tag);
        }
    });
}

function shimDwrThenResolve(configuration: BrokerConfiguration, resolve: Function) {
    if (configuration.fixTz) {
        (function(dwr) {
